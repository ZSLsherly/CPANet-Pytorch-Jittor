...
class cpanet(nn.Module):
    ...
    def forward(self,
                x,
                s_x=torch.FloatTensor(4, 1, 3, 200, 200).cuda(),
                s_y=torch.FloatTensor(4, 1, 200, 200).cuda(),
                y=None):
            ...
            with torch.no_grad():
                ...
                if self.vgg:
                    supp_feat_2 = F.interpolate(supp_feat_2,
                                                ...
                                                mode='bilinear',
                                                align_corners=True)
            supp_gt_down = F.interpolate(supp_gt, size=(supp_feat_3.size(2), supp_feat_3.size(3)), mode='bilinear',align_corners=True)
            gt_down_list.append(supp_gt_down)

            supp_feat = torch.cat([supp_feat_3, supp_feat_2], dim=1)
            supp_feat = self.down_supp(supp_feat)  # [4,256,25,25]
            supp_feat_list.append(supp_feat)


            supp_feat_mask = supp_feat * supp_gt_down
            supp_a = self.REF(supp_feat_mask)  # [1,256,1,1]
        ...
        query_supp = torch.cat([supp_ap, query_feat], dim=1)
        query_out = self.conv_2T1(query_supp)  # 1,256,200,200
