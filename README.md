...
在消融实验中，为了控制变量并便于对比，均以 **VGG16** 作为 Backbone。  
另外，由于时间限制，所有实验均基于 **1-shot、fold-0** 数据集进行训练与评测。
...
建议严格遵循官方推荐的主流环境（Python 版本、CUDA 版本、依赖库版本），避免因环境差异导致难以定位的异常。  
目前 Jittor 的警告和错误提示机制尚不完善，部分潜在问题不会以异常形式抛出，增加调试难度。排查环境和配置问题是调试过程中的重点。
